...
function capture_data(qc::QCBoard, number_of_frames::Int)::PixelVector
    ...
    for i = 1:(words÷packet)
        ...
        data[start_idx:end_idx] =
            read_from_block_pipe_out(qc, FIFO_OUT, packet, packet, packet)
    end
...
function capture_frame(qc::QCBoard)::Union{Matrix{UInt8},Matrix{UInt16}}
    ...
    frame_data = read_from_block_pipe_out(
        qc,
        FIFO_OUT,
        packet,
        frame_size(qc);
        el_size = element_size(qc),
    )
...
function stream_G2_Tint(qc, number_of_tint; plotter::Channel, hdf5_collector::Channel)
    ...
    for i = 1:number_of_tint
        ...
        data_8bits[1:tint_frame_size_bytes] =
            read_from_block_pipe_out(qc.fpga, FIFO_OUT, packet, tint_frame_size_bytes)
...
function stream_G2_components(qc::QCBoard, number_of_tint)
    ...
    for _ = 1:number_of_tint
        ...
        data_8bits[1:tint_frame_size_bytes] =
            read_from_block_pipe_out(qc, FIFO_OUT, packet, tint_frame_size_bytes)
...
function capture_intensity_frames_byte_mode(qc::QCBoard, number_of_frames)
    ...
    for i = 1:capture_iterations
        ...
        data_8bits[1:capture_size] =
            read_from_block_pipe_out(qc.fpga, addr, packet, capture_size, capture_size)
        transfer_ready = 0
    end
...
function capture_G2_Tint(qc::QCBoard, number_of_tint)
    ...
    for i = 1:number_of_tint
        ...
        data_8bits[1:tint_frame_size_bytes] = read_from_block_pipe_out(
            qc.fpga,
            addr,
            packet,
            tint_frame_size_bytes,
            tint_frame_size_bytes,
        )
        transfer_ready = 0
...
function capture_pixel_G2_components(qc, number_of_tint; hdf5_collector::Channel)
    ...
    for i = 1:tint_readout_iterations
        while get_wire_out_value(qc, EP_READY)
        end
        data_8bits[1:readout_size] =
            read_from_block_pipe_out(qc, FIFO_OUT, packet, readout_size)
        transfer_ready = 0
