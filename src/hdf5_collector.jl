...
struct GroupConfig
    name::String
    size::Any # No Idea what a general size Tuple should be for size
    description::String
end
...
function hdf5_collector_thread(
    path::String,
    channel::Channel{H5StreamType{T}};
    description = nothing,
)::Nothing where {T}
...
@testset "Test HDF5 Collector" begin
    ...
    hdf5_task, hdf5_channel =
        hdf5_collector_init("/tmp/test.h5", Matrix{Int}; description = "Test hdf5 file")
