...
function read_from_block_pipe_out(
    qc::QCBoard,
    pipename::BankEnum,
    blksize::Integer,
    frame_size::Integer;
    psize::Union{Nothing,UInt} = nothing,
    el_size = 1,
)::Union{Vector{UInt8},Vector{UInt16}}
    ...
    if !haskey(qc.bank, pipename)
        ...
        data_8bits = OKExtended.read_from_block_pipe_out(
            qc.fpga,
            addr,
            blksize,
            frame_size * el_size,
